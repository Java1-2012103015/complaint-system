...
export async function POST(req: NextRequest) {
  ...
  if (!name || !loginId || !email || !password || !organizationId) {
    return NextResponse.json(
      { error: '이름, 아이디, 이메일, 기관, 비밀번호는 필수입니다.' },
      { status: 400 },
    )
  }
  ...
  try {
    ...
    const [dupEmail, dupLogin, d1Pending] = await Promise.all([
    ...
    const role: 'DISTRIBUTOR_1' | 'DISTRIBUTOR_2' =
      dupEmail?.isTemporary && dupEmail.role === 'DISTRIBUTOR_1'
        ? 'DISTRIBUTOR_1'
        : d1Pending.length > 0
          ? 'DISTRIBUTOR_1'
          : 'DISTRIBUTOR_2'
