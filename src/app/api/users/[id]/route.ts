...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (isAdmin) {
    if (body.role !== undefined && isSelf) {
      return NextResponse.json({ error: '본인 계정의 역할은 변경할 수 없습니다.' }, { status: 400 })
    }

    if (body.role !== undefined) {
      const validRoles = ['ADMIN', 'DISTRIBUTOR_1', 'DISTRIBUTOR_2'] as const
      ...
      if (body.role !== 'ADMIN') {
        ...
        if (target?.role === 'ADMIN') {
          ...
          if (remainingActiveAdmins < 1) {
            return NextResponse.json(
              { error: '활성 관리자가 최소 1명은 남아 있어야 합니다.' },
              { status: 400 },
            )
          }
        }
